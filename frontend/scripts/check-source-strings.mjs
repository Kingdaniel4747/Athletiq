...
const nutritionLocalesDir = join(srcDir, 'nutrition-locales')
...
for (const file of readdirSync(nutritionLocalesDir).filter(f => f.endsWith('.js'))) {
  const pack = (await import(pathToFileURL(join(nutritionLocalesDir, file)).href)).default
  for (const key of Object.keys(pack)) defined.add(key)
}
