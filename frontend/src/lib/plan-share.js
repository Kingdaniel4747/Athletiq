...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR AthletiQ — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { athletiq_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.athletiq_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an AthletiQ plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with AthletiQ'))} · github.com/Kingdaniel4747/Athletiq</footer>
</div></body></html>`
}
