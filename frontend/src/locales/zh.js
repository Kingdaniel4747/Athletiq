...
export default {
  ...
  "This browser doesn't support passkeys — you can still use AthletiQ locally on this device.": '此浏览器不支持通行密钥——你仍可在本设备上本地使用 AthletiQ。',
  ...
  'Passkeys need HTTPS outside localhost. Open AthletiQ through your HTTPS domain; the current address is {0}.': '在 localhost 之外使用通行密钥需要 HTTPS。请通过你的 HTTPS 域名打开 AthletiQ；当前地址为 {0}。',
  'Passkey setup does not match this address. Check the public domain or reverse-proxy headers. Details: {0}': '通行密钥设置与此地址不匹配。请检查公共域名或反向代理标头。详细信息：{0}',
  ...
  'to install AthletiQ as a full-screen app.': '即可将 AthletiQ 安装为全屏应用。',
  ...
  'Rest-timer alerts, even if AthletiQ is closed.': '休息计时提醒，即使 AthletiQ 已关闭。',
  ...
  'A small file a friend imports into their own AthletiQ — routines only, none of your workouts or weigh-ins.': '一个小文件，朋友可以导入到自己的 AthletiQ——只有训练日，不包含你的训练记录或体重。',
