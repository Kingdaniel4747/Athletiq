...
export default function ModeSwitch() {
  ...
  return <div className="mode-switch-wrap">
    <div className="mode-switch" role="group" aria-label={t('App mode')}>
      ...
      <button className={!nutrition ? 'on' : ''} onClick={() => nav('/home')}><Icon name="dumbbell" /><span>Gym</span></button>
      <button className={nutrition ? 'on' : ''} onClick={() => nav('/nutrition')}><Icon name="food" /><span>Health</span></button>
    </div>
  </div>
}
