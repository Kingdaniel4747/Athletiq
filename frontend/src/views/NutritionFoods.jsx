...
export default function NutritionFoods() {
  ...
  return <div className="narrow nutrition-page">
    ...
    <div className="card">
      <div className="row between"><div><h2>Mealie</h2><div className="small muted">{config?.mealie_enabled ? t('Connected through your openGym server') : t('Add MEALIE_URL and MEALIE_API_TOKEN to .env')}</div></div><span className={`status-dot ${config?.mealie_enabled ? 'green' : 'yellow'}`} /></div>
